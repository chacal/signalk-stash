...
// RSA key for testing, copied from https://github.com/auth0/express-openid-connect/blob/master/test/fixture/cert.js
const key = JWK.asKey({
  e: 'AQAB',
  n:
...
export function encryptSessionCookie(payload: any, secret: string) {
  const now = Math.floor(Date.now() / 1000)
  const key = JWK.asKey(deriveKey(secret))
  const cookieValue = JWE.encrypt(JSON.stringify(payload), key, {
    alg: 'dir',
    enc: 'A256GCM',
    iat: now,
    uat: now,
...
function deriveKey(secret: string) {
  return Buffer.from(
    crypto.hkdfSync('sha256', secret, Buffer.alloc(0), 'JWE CEK', 32)
  )
}
