...
const fetchTrackLengthsWithNames = (
  ...
  return Promise.all(
    vessels.reduce<Promise<TrackLengthWithName[]>[]>((acc, { vesselId, name }: VesselData) => {
      years.toArray().forEach((year) => 
      acc.push(fetchTrackLengths(vesselId, year).then((trackLengthsA: TrackLength[]) =>
        trackLengthsA.map(
