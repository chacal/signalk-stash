...
export default function tracksFor(
  ...
  vesselsA
    ...
    .onValue(async vesselA => {
      ...
      const geoJson = await loadTrack(context, enlarge(boundsA.get(), 0.05), zoomA.get())
...
function enlarge(bounds: LatLngBounds, factor: number) {
  ...
  return new LatLngBounds([bounds.getSouth() -latDelta, bounds.getWest()-lngDelta],[bounds.getNorth() + latDelta, bounds.getEast()+lngDelta])
}
